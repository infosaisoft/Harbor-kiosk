...
public class Doctor_HospitalDaoImpl implements Doctor_HospitalDao {
	...
	private static final String HOSPITAl_DO_AND_DONTS="SELECT NAME,DESCRIPTION,IMAGES,VIDEOS,DOCUMENT FROM hospital_dos_donts WHERE HOSPITAL_ID=?";
	...
	public List<HospitalDiseasesBo> getPatientdiesease(long hid) {
		...
		listhdbo = jt.query(get_patient_diseases, new ResultSetExtractor<List<HospitalDiseasesBo>>() {
			...
			@Override
			public List<HospitalDiseasesBo> extractData(ResultSet rs) throws SQLException, DataAccessException {
	...
	public List<HospitalDoAndDontsBo> getAllDoAndDontdByHID(long id) {
		List<HospitalDoAndDontsBo>listdodonts=null;
		
		listdodonts=jt.query(HOSPITAl_DO_AND_DONTS,new ResultSetExtractor<List<HospitalDoAndDontsBo>>() {

			@Override
			public List<HospitalDoAndDontsBo> extractData(ResultSet rs) throws SQLException, DataAccessException {
			   List<HospitalDoAndDontsBo>listdodonts=new ArrayList<>();
			   HospitalDoAndDontsBo hdo=null;
			   while(rs.next()) {
				   hdo=new HospitalDoAndDontsBo();
				   hdo.setName(rs.getString(1));
				   hdo.setImages(rs.getString(2));
				   hdo.setVideos(rs.getString(3));
				   hdo.setDocuments(rs.getString(4));
				   hdo.setDescription(rs.getString(5));
				   listdodonts.add(hdo);
			   }
				return listdodonts;
			}
